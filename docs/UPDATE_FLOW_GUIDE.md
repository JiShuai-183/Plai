...
> **规范版位置**：本文件（仓库 `docs/UPDATE_FLOW_GUIDE.md`）。桌面上的 `Plai更新流程指南.md` 是同一内容的交付副本 —— **改内容请改这里，再同步副本**，不要反过来。
...
本文正文刻意不含机器相关绝对路径。以下为**当前所有者这台机器**的实际位置，仅供所有者本人参考，**不要连同文档一起分发给他人**：

| 资源 | 本机位置 |
|---|---|
| keystore 备份目录 | `E:\LoveLife\plai-signing\`（内含 `debug.keystore` 与 `说明-必读.md`） |
| ECS 私钥 | `E:\LoveLife\DOCX\geeee.pem` |
| 仓库 | `C:\Users\Administrator\Desktop\Plai-master` |
